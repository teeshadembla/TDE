...
import {
    ShieldCheck, ChevronLeft, Users, Crown, UserCheck,
    Shield, Save, RotateCcw, AlertCircle, Check, Lock, RefreshCw
} from 'lucide-react';
...
// ─── Permission group labels — must match backend group enum exactly ──────────
const GROUP_META = {
    registered_features: {
        label: 'Registered User Features',
        description: 'Available to all logged-in users',
    },
    fellow_features: {
        label: 'Fellow Features',
        description: 'Available to chairs and above',
    },
    admin_features: {
        label: 'Admin Features',
        description: 'Available to core members and above',
    },
    super_admin: {
        label: 'Super Admin',
        description: 'Restricted to admin only — never toggle off',
    },
};
    ...
    return (
        <div
            ...
            style={{
                ...
                transition: 'background-color 0.15s', gap: 12,
            }}
        >
            <div style={{ minWidth: 0 }}>
                <p style={{
                    margin: 0, fontFamily: 'Montserrat, sans-serif', fontSize: 13,
                    fontWeight: 500, color: locked ? '#444' : '#ccc', letterSpacing: '-0.01em',
                }}>
                    {permission.label}
                </p>
...
function RolePermissionsPanel({ role, permissions, rolePermissions, onChange, saving, onSave, onReset, hasChanges }) {
    ...
    const groupOrder = ['registered_features', 'fellow_features', 'admin_features', 'super_admin'];

    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>

            {/* Panel header */}
            <div style={{ padding: '24px 28px 20px', borderBottom: '1px solid #1a1a1a', background: meta.bg }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div>
                            <h2 style={{
                                ...
                                fontWeight: 700, fontSize: 18, color: '#f5f5f5', letterSpacing: '-0.02em',
                            }}>
                                {meta.label}
                            </h2>
                    ...
                    {meta.locked ? (
                        <div style={{
                            ...
                            backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid #222',
                        }}>
                            <Lock size={12} color="#555" />
                            <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#555', fontWeight: 600 }}>
                                NON-EDITABLE
                            </span>
                        </div>
                    ) : (
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button
                                ...
                                style={{
                                    ...
                                    transition: 'all 0.15s', opacity: hasChanges ? 1 : 0.5,
                                }}
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    backgroundColor: hasChanges && !saving ? meta.accent : '#1a1a1a',
                                    cursor: hasChanges && !saving ? 'pointer' : 'not-allowed',
...
const RolePermissions = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // Build complete map for every role using the master permissions list
            // Falls back to false for any key not in the stored config
            const built = {};
            ...
            setFetchError(
                err.response?.status === 403
                    ...
                    : 'Failed to load permissions. Check your connection and try again.'
            );
        } finally {
            setLoading(false);
        }
    ...
    const handleSave = async (role) => {
        try {
            ...
            // Update snapshot so hasChanges() returns false after save
            ...
            setSaveError(
                err.response?.data?.msg || 'Failed to save. Please try again.'
            );
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                    <div style={{
                        width: 40, height: 40, border: '2px solid #1a1a1a',
                        borderTopColor: '#004aad', borderRadius: '50%',
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', maxWidth: 360 }}>
                    <AlertCircle size={36} color="#f87171" strokeWidth={1.5} style={{ margin: '0 auto 16px' }} />
                    <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 14, color: '#f87171', margin: '0 0 20px' }}>
                        {fetchError}
                    </p>
                    <button
                        onClick={fetchData}
                        style={{
                            ...
                            backgroundColor: '#004aad', color: '#fff', cursor: 'pointer',
    ...
    return (
        <div
            className='pt-[100px]'
            style={{ minHeight: '100vh', backgroundColor: '#080808', display: 'flex', flexDirection: 'column' }}
        >
            {/* Top bar */}
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{
                        ...
                        fontSize: 14, color: '#f0f0f0', letterSpacing: '-0.01em',
                    }}>
                        Roles & Permissions
                    </span>
                </div>
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    <p style={{
                        ...
                        fontWeight: 700, color: '#333', textTransform: 'uppercase', letterSpacing: '0.12em',
                    }}>
                        Roles
                    </p>
                            ...
                            >
                                ...
                                <div style={{ flex: 1, textAlign: 'left' }}>
                                    <p style={{
                                        ...
                                        color: active ? '#f0f0f0' : '#555', letterSpacing: '-0.01em',
                                    }}>
                                        {meta.label}
                                    </p>
                                </div>
                                ...
                                {meta.locked && <Lock size={11} color="#333" />}
                            </button>
                        );
                    })}
                    ...
                    {/* Legend */}
                    <div style={{
                        margin: '24px 18px 0', padding: '14px', borderRadius: 8,
                        backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid #161616',
                    }}>
                        <p style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
                            ...
                            <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 11, color: '#444' }}>
                                Unsaved changes
                            </span>
                        </div>
